...
def main() -> None:
    ap = argparse.ArgumentParser(description="Plot Macro-F1 vs Epoch for STFT-ResNet-18")
    ...
    out_png = (
        ...
        else csv_path.parent / "macro_f1_epoch_single_stft_resnet_18.png"
    )
    out_svg = (
        Path(args.out_svg).resolve()
        if args.out_svg.strip()
        else csv_path.parent / "macro_f1_epoch_single_stft_resnet_18.svg"
    )
