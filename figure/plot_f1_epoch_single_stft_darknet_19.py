...
def main() -> None:
    ap = argparse.ArgumentParser(description="Plot Macro-F1 vs Epoch for STFT-DarkNet-19")
    ...
    ap.add_argument("--y_min", type=float, default=0.20)
    ...
    out_png = (
        ...
        else csv_path.parent / "macro_f1_epoch_single_stft_darknet_19.png"
    )
    out_svg = (
        Path(args.out_svg).resolve()
        if args.out_svg.strip()
        else csv_path.parent / "macro_f1_epoch_single_stft_darknet_19.svg"
    )
