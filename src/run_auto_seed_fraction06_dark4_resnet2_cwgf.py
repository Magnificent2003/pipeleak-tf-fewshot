...
def main():
    ...
    if args.out_root.strip():
        ...
        out_root = project_root / "runs" / f"auto_seed_fraction06_dark4_resnet2_cwgf_{now_str()}"
    ...
    summary_csv = csv_dir / "summary_seed06_backbone_cwgf_f1.csv"
    ...
    print("========== Auto Seed (fraction=0.6): Dark4 + ResNet2 + CWGF ==========")
    ...
    for seed_name, seed_value in seed_list:
        ...
        # build stft from fractioned raw
        t0 = time.time()
        ts0 = dt.datetime.now().isoformat(timespec="seconds")
        status = "ok"
        err = ""
        ...
        jobs = [
            {
                ...
                "args": [
                    "--data_root", str(seed_stft),
                    "--save_dir", str(seed_ckpt),
                    "--log_dir", str(csv_dir),
                    "--num_workers", "0",
                ],
        ...
        for job in jobs:
            ...
            try:
                ...
                if not dry_run:
                    ...
                    if job["step"] == "darknet4":
                        dark_ckpt = str(new_ckpt.resolve())
                        dark_csv_tagged = tagged_csv
                    elif job["step"] == "resnet2":
                        res_ckpt = str(new_ckpt.resolve())
                        res_csv_tagged = tagged_csv
            except Exception as e:
                status = "fail"
                err = str(e)
                seed_failed = True
        ...
        try:
            before_csv = list(csv_dir.glob("metrics_fuse_cwgf_dark4parent_resnet2_*.csv"))
            cmd = [
                python_exe,
                "run_fuse_cwgf_dark4parent_resnet2.py",
                "--data_root", str(seed_stft),
                "--save_dir", str(seed_ckpt),
                "--log_dir", str(csv_dir),
                "--ckpt_resnet2", str(res_ckpt),
                "--ckpt_darknet4", str(dark_ckpt),
                "--num_workers", "0",
            ]
            run_cmd(cmd, cwd=src_dir, stdout_file=stdout_dir / f"{seed_tag}__fuse_cwgf.log", dry_run=dry_run)
            if not dry_run:
                new_csv = pick_new_file(csv_dir, "metrics_fuse_cwgf_dark4parent_resnet2_*.csv", before_csv)
                fuse_csv_tagged = tag_csv_name(new_csv, seed_name, seed_value, "fuse_cwgf_dark4parent_resnet2")
        except Exception as e:
            status = "fail"
            err = str(e)
            seed_failed = True
        ...
        try:
            if not dry_run:
                dark_best = read_best_row(dark_csv_tagged)
                res_best = read_best_row(res_csv_tagged)
                fuse_last = read_single_row_csv(fuse_csv_tagged)

                row["darknet19_4cls_parent_f1"] = f"{parse_metric_from_row(dark_best, ['test_parent_f1']):.6f}"
                row["resnet18_2cls_f1"] = f"{parse_metric_from_row(res_best, ['test_f1']):.6f}"
                row["cwgf_status"] = str(fuse_last.get("degeneracy", "")).strip()
                row["cwgf_f1"] = f"{float(str(fuse_last.get('test_fuse_f1', '')).strip()):.6f}"
                row["darknet_csv"] = dark_csv_tagged.name
                row["resnet_csv"] = res_csv_tagged.name
                row["cwgf_csv"] = fuse_csv_tagged.name
            else:
                row["darknet19_4cls_parent_f1"] = "DRY_RUN"
