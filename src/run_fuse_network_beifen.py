...
# def find_best_thr(p, y, lo=0.2, hi=0.8, step=0.01):
#     best, tau_best = -1.0, 0.5
#     tau = lo
#     while tau <= hi + 1e-12:
#         m = bin_metrics_from_probs(p, y, tau)
#         if m["f1"] > best:
#             best, tau_best = m["f1"], tau
#         tau += step
#     return tau_best, best
...
def main():
    ...
    best_w, best_f1_w = 0.0, -1.0
    for w in np.linspace(0.0, 1.0, 21):
        p_va = w * pr_va + (1-w) * pc_va
        _, f1_va = find_best_thr(p_va, y_val_arr, lo=0.01, hi=0.99, step=0.01)
        if f1_va > best_f1_w:
            best_f1_w, best_w = f1_va, w

    print(f"[GRID] best_w={best_w:.3f} val_f1={best_f1_w:.4f}")
    # test
    p_te = best_w * pr_te + (1-best_w) * pc_te
    tau_te, f1_te = find_best_thr(p_te, np.load(os.path.join(root,"y_test.npy")), lo=0.01, hi=0.99, step=0.01)
    print(f"[GRID] TEST w={best_w:.3f} tau={tau_te:.2f} f1={f1_te:.4f}")
    ...
    for ep in range(1, args.gate_epochs + 1):
        ...
        opt.zero_grad()
        g_tr = gate(Zv_tr_t)
        p_fuse_tr = g_tr * Pr_tr_t + (1.0 - g_tr) * Pc_tr_t
        p_fuse_tr = torch.clamp(p_fuse_tr, 1e-6, 1.0 - 1e-6)
        lambda_ent = 0.1   # try 0.05, 0.1, 0.2
        bce = bce_on_probs(p_fuse_tr, y_tr_t, w_pos=w1, w_neg=w0)
        loss = bce + lambda_ent * entropy_pos(p_fuse_tr)
        loss.backward()
        ...
        if ep % 20 == 0 or ep == 1:
            ...
            with torch.no_grad():
                # 用验证集计算 f1
                g_va = gate(Zv_va_t)
                p_fuse_va = g_va * pr_va_t + (1.0 - g_va) * pc_va_t
                p_fuse_va = torch.clamp(p_fuse_va, 1e-9, 1.0 - 1e-9)
                ...
                print(f"Epoch {ep:03d} | gate_loss={loss.item():.4f} | val@{tau:.2f} f1={f1_:.4f}")
        ...
        #     gate.eval()
        #     with torch.no_grad():
        #         # 用训练集计算 f1（以训练集F1最高为目标）
        #         g_tr_eval = gate(Zv_tr_t)
        #         p_fuse_tr_eval = g_tr_eval * Pr_tr_t + (1.0 - g_tr_eval) * Pc_tr_t
        #         p_fuse_tr_eval = torch.clamp(p_fuse_tr_eval, 1e-9, 1.0 - 1e-9)

        #         p_fuse_tr_np = p_fuse_tr_eval.detach().cpu().numpy()
        #         tau, f1_ = find_best_thr(p_fuse_tr_np, y_tr_arr,
        #                                  args.thr_lo, args.thr_hi, args.thr_step)
        #         print(f"Epoch {ep:03d} | gate_loss={loss.item():.4f} | train@{tau:.2f} f1={f1_:.4f}")
    ...
    gt = gate(Zt).detach().cpu().numpy()

    y_test_arr = np.load(os.path.join(root, "y_test.npy"))

    p_fuse_te = gt * pr_te + (1.0 - gt) * pc_te
    p_fuse_te = np.clip(p_fuse_te, 1e-9, 1.0 - 1e-9)
