...
# InceptionTime-1d + MFCC + HHT
MODEL_SCRIPTS: List[str] = [
    "run_base_inception-1d_2cls.py",
    "run_base_inception-1d_4cls.py",
    "run_base_mfcc_svm_2cls.py",
    "run_base_mfcc_svm_4cls.py",
    "run_base_mfcc_mlp_2cls.py",
    "run_base_mfcc_mlp_4cls.py",
    "run_base_hht_svm_2cls.py",
    "run_base_hht_svm_4cls.py",
    "run_base_hht_mlp_2cls.py",
    "run_base_hht_mlp_4cls.py",
]
...
def main():
    ...
    if args.out_root.strip():
        ...
        out_root = project_root / "runs" / f"auto_benchmark_inception_mfcc_hht_{now_str()}"
    csv_dir = out_root / "csv"
    ckpt_root = out_root / "checkpoints"
    hht_cache_root = out_root / "hht_cache"
    os.makedirs(csv_dir, exist_ok=True)
    os.makedirs(ckpt_root, exist_ok=True)
    os.makedirs(hht_cache_root, exist_ok=True)

    summary_path = csv_dir / "summary_auto_benchmark_inception_mfcc_hht.csv"
    summary_rows: List[Dict[str, str]] = []

    print("===== Auto Benchmark (Inception/MFCC/HHT) =====")
    ...
    for seed_name, seed_val in seed_list:
        ...
        seed_hht_cache_dir = hht_cache_root / seed_tag
        os.makedirs(seed_ckpt_dir, exist_ok=True)
        os.makedirs(seed_hht_cache_dir, exist_ok=True)
        print(f"\n==== Start Seed: {seed_tag} ====")
        ...
        for i, script_name in enumerate(MODEL_SCRIPTS, start=1):
            ...
            # HHT 脚本增加 per-seed cache 隔离，避免不同 split 复用缓存。
            if "run_base_hht_" in script_name:
                cmd += ["--hht_cache_dir", str(seed_hht_cache_dir)]
    ...
    print(f"HHT cache dir : {hht_cache_root}")
