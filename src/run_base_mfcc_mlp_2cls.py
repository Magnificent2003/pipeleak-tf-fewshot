...
class MLP2(nn.Module):
    def __init__(self, in_dim, hidden=128, p_drop=0.2):
        ...
        self.net = nn.Sequential(
            ...
            nn.Dropout(p_drop),
            nn.Linear(hidden, 1),  # 输出 1 个 logit
        )
...
def main():
    ...
    in_dim = int(Ftr.shape[1])  # 2 * n_mfcc，默认 40
    ...
    log_path = os.path.join(args.log_dir, f"metrics_mlp_mfcc_2cls_{stamp}.csv")
    ...
    ckpt_path = os.path.join(args.save_dir, f"mlp_mfcc_2cls_best_{stamp}.pt")
    torch.save(
        {
            "model": "MLP2_MFCC",
