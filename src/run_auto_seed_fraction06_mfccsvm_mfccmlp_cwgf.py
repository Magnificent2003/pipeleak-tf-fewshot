...
def apply_train_fraction_raw_4cls(raw_root: Path, fraction: float, seed: int) -> Tuple[int, int]:
    ...
    classes = sorted(int(c) for c in np.unique(y4))
    picked = []
    for cls in classes:
        idx = np.where(y4 == cls)[0]
...
def main():
    ...
    if args.out_root.strip():
        ...
        out_root = project_root / "runs" / f"auto_seed_fraction06_mfccsvm_mfccmlp_cwgf_{now_str()}"
    ...
    summary_csv = csv_dir / "summary_seed06_mfccsvm_mfccmlp_cwgf_f1.csv"
    ...
    print("========== Auto Seed (fraction=0.6): MFCC-SVM4 + MFCC-MLP4 + CWGF ==========")
    ...
    for seed_name, seed_value in seed_list:
        ...
        try:
            if not dry_run:
                n_keep, n_total = apply_train_fraction_raw_4cls(seed_raw, TRAIN_FRACTION, seed_value)
        ...
        jobs = [
            {
                "step": "mfcc_svm4",
                "script": "run_base_mfcc_svm_4cls.py",
                "args": [
                    "--data_root",
                    str(seed_raw),
                    "--save_dir",
                ...
                "csv_pat": "metrics_svm_mfcc_4cls_*.csv",
                "ckpt_pat": "svm_mfcc_4cls_best_*.joblib",
                "tag": "mfcc_svm4",
            },
            {
                "step": "mfcc_mlp4",
        ...
        svm_ckpt = ""
        mlp_ckpt = ""
        svm_csv_tagged: Path = None
        mlp_csv_tagged: Path = None
        ...
        for job in jobs:
            ...
            try:
                ...
                if not dry_run:
                    ...
                    if job["step"] == "mfcc_svm4":
                        svm_ckpt = str(new_ckpt.resolve())
                        svm_csv_tagged = tagged_csv
                    elif job["step"] == "mfcc_mlp4":
                        mlp_ckpt = str(new_ckpt.resolve())
                        mlp_csv_tagged = tagged_csv
            except Exception as e:
                status = "fail"
                err = str(e)
                seed_failed = True
        ...
        status = "ok"
        err = ""
        fuse_csv_tagged: Path = None
        try:
            before_csv = list(csv_dir.glob("metrics_fuse_cwgf_mfccsvm_mfccmlp_4cls_*.csv"))
            cmd = [
                python_exe,
                "run_fuse_cwgf_mfccsvm_mfccmlp_4cls.py",
                "--data_root_raw",
                str(seed_raw),
                "--save_dir",
                str(seed_ckpt),
            ...
            if not dry_run:
                new_csv = pick_new_file(csv_dir, "metrics_fuse_cwgf_mfccsvm_mfccmlp_4cls_*.csv", before_csv)
                fuse_csv_tagged = tag_csv_name(new_csv, seed_name, seed_value, "fuse_cwgf_mfccsvm_mfccmlp4")
        except Exception as e:
            status = "fail"
            err = str(e)
            seed_failed = True
        ...
        try:
            if not dry_run:
                svm_best = read_best_row(svm_csv_tagged)
                mlp_best = read_best_row(mlp_csv_tagged)
                fuse_last = read_single_row_csv(fuse_csv_tagged)

                row["mfcc_svm_4cls_f1"] = f"{parse_metric_from_row(svm_best, ['test_macro_f1']):.6f}"
                row["mfcc_mlp_4cls_f1"] = f"{parse_metric_from_row(mlp_best, ['test_macro_f1']):.6f}"
                row["cwgf_status"] = str(fuse_last.get("degeneracy", "")).strip()
                row["cwgf_f1"] = f"{float(str(fuse_last.get('test_fuse_macro_f1', '')).strip()):.6f}"
                row["svm_csv"] = svm_csv_tagged.name
                row["mlp_csv"] = mlp_csv_tagged.name
                row["cwgf_csv"] = fuse_csv_tagged.name
            else:
                row["mfcc_svm_4cls_f1"] = "DRY_RUN"
                row["mfcc_mlp_4cls_f1"] = "DRY_RUN"
