...
def parse_metric_from_csv(csv_path: Path, metric_cols: Sequence[str]) -> float:
    ...
    best_rows = [r for r in rows if str(r.get("episode", "")).strip().lower() == "best"]
...
class ProtoTrainSnapshot:
    def __init__(self, data_root: Path) -> None:
        self.data_root = data_root
        self.p_y4_train = data_root / "y4_train.npy"
        self.p_png_train = data_root / "png_train"
        self.p_png_backup = data_root / "png_train__full_backup_auto_proto"
...
def aggregate_mean_sd_by_fraction(rows: Sequence[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    for r in rows:
        if r.get("status", "") != "ok":
            continue
        frac = r.get("fraction", "")
        val = r.get("proto_darknet_4cls_macro_f1", "")
        if frac == "" or val == "":
            continue
...
def main() -> None:
    ...
    ap.add_argument("--proto_episodes", type=int, default=cfg.PROTO_EPISODES)
    ap.add_argument("--proto_eval_every", type=int, default=cfg.PROTO_EVAL_EVERY)
    ap.add_argument("--proto_val_episodes", type=int, default=cfg.PROTO_VAL_EPISODES)
    ap.add_argument("--proto_test_episodes", type=int, default=cfg.PROTO_TEST_EPISODES)
    ...
    if args.out_root.strip():
        ...
        out_root = project_root / "runs" / f"auto_seed_fraction_proto_darknet4_{now_str()}"
    ...
    summary_path = csv_dir / "summary_seed_fraction_proto_darknet4_macro_f1.csv"
    mean_sd_path = csv_dir / "summary_fraction_proto_darknet4_macro_f1_mean_sd.csv"
    ...
    print("========== Auto Seed×Fraction ProtoDarkNet19 (4cls) ==========")
    ...
    for seed_name, seed_value in seeds:
        ...
        try:
            ...
            snapshot = ProtoTrainSnapshot(data_root=data_root) if not dry_run else None
            ...
            for frac in fracs:
                ...
                try:
                    cmd = [
                        ...
                        str(seed_value),
                        "--episodes",
                        str(int(args.proto_episodes)),
                        "--eval_every",
                        str(int(args.proto_eval_every)),
                        "--val_episodes",
                        str(int(args.proto_val_episodes)),
                        "--test_episodes",
                        str(int(args.proto_test_episodes)),
                    ]

                    before_csv = list(csv_dir.glob("metrics_proto_darknet19_4cls_*.csv"))
                    run_cmd(
                        cmd,
                        cwd=src_dir,
                        stdout_file=stdout_dir / f"{seed_tag}__{frac_tag(frac)}__proto_darknet_4cls.log",
                        dry_run=dry_run,
                    )

                    if not dry_run:
                        new_csv = pick_new_file(csv_dir, "metrics_proto_darknet19_4cls_*.csv", before_csv)
                        tagged = tag_csv_name(new_csv, seed_name, seed_value, frac, "proto_darknet_4cls")
                        metric = parse_metric_from_csv(tagged, ["test_f1"])
                        row["proto_darknet_4cls_macro_f1"] = f"{metric:.6f}"
                except Exception as e:
                    status, err = "fail", str(e)
                ...
                if row["status"] == "ok":
                    print(f"[DONE] {seed_tag} fr={frac:.1f} | macro-f1={row['proto_darknet_4cls_macro_f1']}")
                else:
